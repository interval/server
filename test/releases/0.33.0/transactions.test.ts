...
import { sleep, consoleUrl, config } from '../../_setup'
...
test.skip(process.env.SDK_VERSION !== '0.33.0')
...
test.describe.parallel('Basic transactions tests', () => {
  ...
  test('io.display.metadata', async ({ page, transactions }) => {
    ...
    for (const heading of [
      ...
      await expect(page.locator('dt:has-text("Is true")')).toBeVisible()
      await expect(page.locator('dd:has-text("true")')).toBeVisible()
      await expect(page.locator('dt:has-text("Is false")')).toBeVisible()
      await expect(page.locator('dd:has-text("false")')).toBeVisible()
      await expect(page.locator('dt:has-text("Is null")')).toBeVisible()
      await expect(page.locator('dt:has-text("Is empty")')).toBeVisible()
      await expect(page.locator('dt:has-text("Is long string")')).toBeVisible()
      await expect(
        page.locator(
          'dd:has-text("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed sit amet quam in lorem")'
        )
      ).toBeVisible()
      await expect(page.locator('dt:has-text("Is number 15")')).toBeVisible()
      await expect(page.locator('dd:has-text("15")')).toBeVisible()
      await expect(page.locator('dt:has-text("Is string")')).toBeVisible()
      await expect(page.locator('dd:has-text("Hello")')).toBeVisible()
      await expect(page.locator('dt:has-text("Action link")')).toBeVisible()
      await expect(page.locator('dd a:has-text("Click me")')).toBeVisible()
      await transactions.continue()
    }
  ...
  test('optional', async ({ page, transactions }) => {
    ...
    for (const field of fields) {
      switch (field) {
        ...
        case 'Select single':
        case 'Search':
          await page.locator(`label:has-text("${field}")`).click()
          break
        default:
  ...
  test.describe('Unlisted', () => {
    ...
    test('Group', async ({ page, transactions }) => {
      ...
      await page.goto(`${await consoleUrl()}/unlisted_router`)
      await expect(page.locator('h2:has-text("Unlisted router")')).toBeVisible()
      await transactions.run('unlisted_router/unlisted_listed')
      await transactions.expectSuccess()
    })
  })
})
