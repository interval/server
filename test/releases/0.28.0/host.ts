...
import ExperimentalInterval, {
  ActionGroup,
} from '@interval/sdk/dist/experimental'
...
export default async function setupHost() {
  ...
  const interval = new Interval({
    ...
    actions: {
      ...
      upload: async io => {
        const file = await io.experimental.input.file('Upload a file')
      ...
      redirect: new ActionGroup({
        ...
        actions: {
          ...
          redirect_action: async io => {
            const [action, paramsStr] = await io.group([
  ...
  const dynamicGroup = new ActionGroup({
    name: 'Dynamic',
    actions: {
      placeholder: async () => {
  ...
  experimental.actions.add('before_listen', async () => {
    return 'Hello, from the past'
  })

  experimental.listen().then(() => {
    experimental.addGroup('dynamic_group', dynamicGroup)
    experimental.actions.add('after_listen', async () => {
